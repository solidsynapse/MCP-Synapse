...
from src.providers.groq_client import GroqHTTPError, GroqProviderClient
...
def test_factory_create_groq_returns_client(tmp_path: Path):
    agent = _make_agent()
    ...
    client = ProviderFactory.create("groq", ctx)
...
def test_groq_http_error_raises_and_logs_error_row(tmp_path: Path, status_code: int):
    agent = _make_agent()
    key_file = tmp_path / "key.txt"
    key_file.write_text("k", encoding="utf-8")
    db_path = tmp_path / "usage.db"
...
def test_groq_streaming_raises_and_logs_error_row(tmp_path: Path):
    agent = _make_agent()
