...
from src.providers.azure_openai_client import AzureOpenAIProviderClient, AzureOpenAIHTTPError
...
def test_factory_create_azure_openai_returns_azure_client(tmp_path: Path):
    agent = _make_agent()
    ...
    client = ProviderFactory.create("azure_openai", ctx)
    assert isinstance(client, AzureOpenAIProviderClient)


def test_azure_openai_success_logs_usage_row(tmp_path: Path):
    agent = _make_agent()
    key_file = tmp_path / "key.txt"
    key_file.write_text("k", encoding="utf-8")
    ...
    with patch("src.providers.azure_openai_client.urllib.request.urlopen") as mock_urlopen:
...
def test_azure_openai_http_error_raises_and_logs_error_row(tmp_path: Path, status_code: int):
    agent = _make_agent()
    key_file = tmp_path / "key.txt"
    key_file.write_text("k", encoding="utf-8")
    db_path = tmp_path / "usage.db"
    ...
    with patch("src.providers.azure_openai_client.urllib.request.urlopen") as mock_urlopen:
        ...
        with pytest.raises(AzureOpenAIHTTPError) as excinfo:
...
def test_azure_openai_streaming_raises_and_logs_error_row(tmp_path: Path):
    agent = _make_agent()
    ...
    with patch("src.providers.azure_openai_client.urllib.request.urlopen") as mock_urlopen:
        with pytest.raises(NotImplementedError) as excinfo:
            manager.execute_request_v1(agent["id"], "hello")
        assert str(excinfo.value) == "azure_openai streaming not implemented"
