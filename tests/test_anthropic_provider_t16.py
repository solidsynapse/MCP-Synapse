...
from src.providers.anthropic_client import AnthropicHTTPError, AnthropicProviderClient
...
def test_factory_create_anthropic_returns_client(tmp_path: Path):
    agent = _make_agent()
    ...
    client = ProviderFactory.create("anthropic", ctx)
    assert isinstance(client, AnthropicProviderClient)


def test_anthropic_success_logs_usage_row(tmp_path: Path):
    agent = _make_agent()
    key_file = tmp_path / "key.txt"
    key_file.write_text("k", encoding="utf-8")
    ...
    with patch("src.providers.anthropic_client.urllib.request.urlopen") as mock_urlopen:
...
def test_anthropic_http_error_raises_and_logs_error_row(tmp_path: Path, status_code: int):
    agent = _make_agent()
    key_file = tmp_path / "key.txt"
    key_file.write_text("k", encoding="utf-8")
    db_path = tmp_path / "usage.db"
    ...
    with patch("src.providers.anthropic_client.urllib.request.urlopen") as mock_urlopen:
        ...
        with pytest.raises(AnthropicHTTPError) as excinfo:
...
def test_anthropic_streaming_raises_and_logs_error_row(tmp_path: Path):
    agent = _make_agent()
    ...
    with patch("src.providers.anthropic_client.urllib.request.urlopen") as mock_urlopen:
        with pytest.raises(NotImplementedError) as excinfo:
            manager.execute_request_v1(agent["id"], "hello")
        assert str(excinfo.value) == "anthropic streaming not implemented"
