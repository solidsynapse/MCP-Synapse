...
def _make_agent(*, provider_id: str, model_id: str) -> dict[str, Any]:
    return {
        "id": "agent-offline-pr5-1",
        "name": "Offline Fake Provider Agent PR5",
...
def compute_pr5_determinism_hash(prompt: str = "hello offline pr5") -> str:
    class FakeProviderClient:
        provider_id = "groq_fake"

        def __init__(self) -> None:
            self.model_id = "groq-fake-model-1"
            self.returned_snapshot: dict[str, Any] | None = None

        def generate_content(self, prompt: str, *, stream: bool = False) -> dict[str, Any]:
            if stream:
                raise NotImplementedError("groq_fake streaming not implemented")
            out = {
                "text": f"GROQ_FAKE:{prompt}",
                "tokens_input": 1,
                "tokens_output": 1,
                "cost_usd": 0.0,
            }
    ...
    prev_factory = ProviderFactory._registry.get("groq_fake")
    ProviderFactory.register("groq_fake", _factory)
    try:
        agent = _make_agent(provider_id="groq_fake", model_id=fake.model_id)
        ...
        assert result1["provider"] == "groq_fake"
        ...
        if prev_factory is None:
            ProviderFactory._registry.pop("groq_fake", None)
        else:
            ProviderFactory.register("groq_fake", prev_factory)


def test_pr5_i0_offline_fake_provider_groq_end_to_end_deterministic():
    h1 = compute_pr5_determinism_hash()
    h2 = compute_pr5_determinism_hash()
    assert h1 == h2
