param(
    ...
    [Parameter(Mandatory = $false)][string]$Prompt = "Reply with exactly three words: OpenAI is reachable.",
    [Parameter(Mandatory = $false)][string]$CredentialsPath = ".\\data\\credentials\\openai_api_key.txt"
)
...
$evidenceRoot = Join-Path (Join-Path (Join-Path $repoRoot "docs") "evidence") ("G_P3_openai_feasibility_{0}" -f $timestamp)
...
$commandsRan += "python .\\tools\\openai_probe.py --model <model> --prompt <prompt> --credentials-path <path> [--base-url <url>]"
...
_WriteTextFile -Path (Join-Path $evidenceRoot "changed_files.txt") -Lines @(
    ...
    "tools/openai_probe.py",
    "tools/run_openai_feasibility.ps1"
)
_WriteTextFile -Path (Join-Path $evidenceRoot "rollback.txt") -Lines @(
    "Remove src/providers/openai_client.py",
    "Revert src/providers/factory.py",
    "Remove tools/openai_probe.py",
    "Remove tools/run_openai_feasibility.ps1"
)
...
$probeOutPath = Join-Path $evidenceRoot "openai_probe.out.txt"
$argsList = @(".\\tools\\openai_probe.py", "--model", $Model, "--prompt", $Prompt, "--credentials-path", $CredentialsPath)
if ($BaseUrl.Trim()) {
    $argsList = @(".\\tools\\openai_probe.py", "--base-url", $BaseUrl, "--model", $Model, "--prompt", $Prompt, "--credentials-path", $CredentialsPath)
}
...
$probeLines += ("CREDENTIALS_PATH={0}" -f $CredentialsPath)
$probeLines += "---"
...
$probeFiles = @(
    (Join-Path (Join-Path $repoRoot "src") "providers\\openai_client.py"),
    (Join-Path $repoRoot "tools\\openai_probe.py")
)
