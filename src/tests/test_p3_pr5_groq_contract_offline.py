...
from src.providers.groq_contract_pr5 import (
    FakeGroqProviderClient,
...
def test_pr5_contract_surface_and_request_shape(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_network_guard(monkeypatch)
    payload = build_groq_chat_payload(model_id="llama3", prompt="hi")
    assert payload == {
        "model": "llama3",
...
def test_pr5_fake_provider_is_deterministic_and_offline(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_network_guard(monkeypatch)
    client = FakeGroqProviderClient(model_id="m1")
    a = client.generate_content("Q")
    b = client.generate_content("Q")
    assert a == b
    assert a["text"] == "GROQ_OFFLINE[m1]:Q"
    assert a["cost_usd"] is None


def test_pr5_fake_provider_streaming_is_not_supported(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_network_guard(monkeypatch)
    client = FakeGroqProviderClient(model_id="m1")
