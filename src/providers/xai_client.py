...
class XAIProviderClient:
    ...
    def generate_content(self, prompt: str, *, stream: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            raise XAIHTTPError(status_code, f"HTTP {status_code}: {msg}") from http_exc
        except Exception as exc:
            raise XAIError(f"xAI unreachable at {self._base}: {exc}") from exc
    ...
    def _parse_response(self, raw: bytes) -> dict[str, Any]:
        try:
            ...
            raise XAIResponseError(f"Invalid JSON response: {exc}") from exc
        ...
        try:
            ...
            raise XAIResponseError(f"Malformed response: {exc}") from exc
