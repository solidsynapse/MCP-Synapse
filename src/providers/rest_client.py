...
_GENERIC_RESPONSE_FIELD_MESSAGE = "Configured response field was not found"
_GENERIC_MISSING_CREDENTIALS_FILE = "Credentials file does not exist"
_GENERIC_EMPTY_CREDENTIALS_FILE = "Credentials file is empty"
...
class RestAPIProviderClient:
    ...
    def generate_content(self, prompt: str, *, stream: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            raise RestAPIHTTPError(status_code, f"HTTP {status_code}: {_GENERIC_HTTP_MESSAGE}") from http_exc
        except Exception as exc:
            raise RestAPIError(_GENERIC_UNREACHABLE_MESSAGE) from exc

        try:
            parsed = json.loads(raw.decode("utf-8"))
        except Exception as exc:
            raise RestAPIResponseError(_GENERIC_INVALID_JSON_MESSAGE) from exc

        value = self._extract_response_field(parsed, self._response_field)
        if value is None:
            raise RestAPIResponseError(_GENERIC_RESPONSE_FIELD_MESSAGE)
    ...
    def _read_secret(self) -> str:
        ...
        if not path.exists():
            raise ValueError(_GENERIC_MISSING_CREDENTIALS_FILE)
        secret = path.read_text(encoding="utf-8").strip()
        if not secret:
            raise ValueError(_GENERIC_EMPTY_CREDENTIALS_FILE)
