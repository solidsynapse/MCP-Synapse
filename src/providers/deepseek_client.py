...
class DeepSeekProviderClient:
    ...
    def __init__(
        ...
        *,
        ...
        base = str(agent.get("deepseek_base_url") or _DEFAULT_BASE_URL).strip()
        ...
        if not (self._base.startswith("https://") or self._base.startswith("http://")):
            raise ValueError("Invalid deepseek_base_url; expected http(s) URL")
    ...
    def generate_content(self, prompt: str, *, stream: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            raise DeepSeekHTTPError(status_code, f"HTTP {status_code}: {_GENERIC_HTTP_MESSAGE}") from http_exc
        except Exception as exc:
            raise DeepSeekError(_GENERIC_UNREACHABLE_MESSAGE) from exc
    ...
    def _parse_response(self, raw: bytes) -> dict[str, Any]:
        try:
            ...
            raise DeepSeekResponseError(_GENERIC_INVALID_JSON_MESSAGE) from exc
        ...
        try:
            ...
            raise DeepSeekResponseError(_GENERIC_MALFORMED_RESPONSE_MESSAGE) from exc
