...
- One supported AI provider account with valid credentials
- Permission to install a desktop app on your machine

The current first-release provider surface includes:
...
Bedrock is not part of the current first-release provider surface.

## 2. Install

Use the official `v0.6.1` release package:

- NSIS installer: `MCP Synapse_0.6.1_x64-setup.exe`
- MSI installer: `MCP Synapse_0.6.1_x64_en-US.msi`
- SHA256 manifest: `SHA256SUMS.txt`

Release page:

- `https://github.com/solidsynapse/MCP-Synapse/releases/tag/v0.6.1`

### SmartScreen and unsigned notice
...
1. Confirm that you downloaded the installer from the official `v0.6.1` release page.
...
```powershell
Get-FileHash 'C:\Path\To\MCP Synapse_0.6.1_x64-setup.exe' -Algorithm SHA256
```

Or, if you downloaded the MSI:

```powershell
Get-FileHash 'C:\Path\To\MCP Synapse_0.6.1_x64_en-US.msi' -Algorithm SHA256
```
...
The Connections surface is where you create, edit, start, stop, and delete provider-backed bridges. It also exposes schema-guided forms, preflight validation, and MCP config export.

When creating a connection, the exact fields depend on the provider you choose. Common examples:
...
- HuggingFace: endpoint or model settings plus the required access token
- Ollama: local base URL and the installed local model name
...
Before you rely on a connection, use **Preflight**. This validates that the current provider form, credentials, and basic runtime reachability are coherent enough for the connection to start.

If preflight fails, fix that first. Do not assume the bridge will behave correctly later if the validation step is already red.
...
The Dashboard is the quick operational view. It shows cost, latency-adjacent operational visibility, health alerts, recent requests, KPI cards, and cost trends so you can see current behavior without opening deeper tools first.
...
Cost visibility stays explicit through the `cost_source` badge:
...
Interceptors currently cover persisted JSON syntax-repair behavior. Budget Guard is monitor-only in the current lane, so it is useful for visibility and planning, not for hard request enforcement yet.
...
1. Re-check the provider-specific fields.
2. Confirm the credential source is the one you intended to use.
3. Verify the selected model, endpoint, project, or deployment name is valid for that provider.
4. Re-run preflight before attempting to start or export the bridge.

### SmartScreen blocker

If Windows blocks launch:

1. Confirm the installer came from the official `v0.6.1` release page.
2. Verify the SHA256 hash.
3. Review the unsigned notice.
4. Proceed only after source and hash both match.

### Provider connection error

If the bridge starts but provider requests fail:

1. Re-check the credential fields for the selected provider.
2. Confirm the model or deployment identifier is valid in your account.
