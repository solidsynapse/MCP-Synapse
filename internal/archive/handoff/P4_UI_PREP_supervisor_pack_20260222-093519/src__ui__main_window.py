...
def _validate_azure_openai_fields(endpoint: str, api_version: str) -> str | None:
    ep = str(endpoint or "").strip()
    av = str(api_version or "").strip()
    if not ep:
        return "azure_endpoint is required"
    if not ep.startswith("https://"):
        return "azure_endpoint must start with https://"
    if ".openai.azure.com" not in ep:
        return "azure_endpoint must contain .openai.azure.com"
    if not av:
        return "azure_api_version is required"
    return None
...
def _update_agent_config_fields(
    ...
    *,
    ...
    pid = str(provider_id or "vertex")
    if pid == "vertex":
        if "provider_id" in updated:
            del updated["provider_id"]
    else:
        updated["provider_id"] = pid

    if pid == "azure_openai":
        err = _validate_azure_openai_fields(str(azure_endpoint or ""), str(azure_api_version or ""))
        if err:
            raise ValueError(err)
        updated["azure_endpoint"] = str(azure_endpoint or "").strip()
        updated["azure_api_version"] = str(azure_api_version or "").strip()
        dep = str(deployment_name or "").strip()
        if dep:
            updated["deployment_name"] = dep
        else:
            if "deployment_name" in updated:
                del updated["deployment_name"]
...
def main(page: ft.Page) -> None:
    ...
    summary_total_requests = ft.Text("0", size=20, weight=ft.FontWeight.BOLD)
    ...
    usage_summary_row = ft.Row(
        controls=[
            build_summary_card("Total Requests", summary_total_requests, "📨"),
    ...
    def apply_usage_filters() -> None:
        ...
        if not rows:
            ...
            summary_total_requests.value = "0"
            ...
            summary_total_requests.value = str(total_requests)
            summary_total_tokens.value = format_compact_number(total_tokens)
            summary_success_rate.value = f"{(total_success / total_requests) * 100.0:.1f}%"
            if latency_values:
    ...
    def build_agent_card(agent: dict) -> ft.Container:
        ...
        def handle_edit(e) -> None:
            ...
            provider_id_field = ft.TextField(
                label="provider_id (optional)",
                width=300,
                value=str(agent.get("provider_id") or agent.get("provider") or ""),
            )
            azure_endpoint_field = ft.TextField(
                label="azure_endpoint",
                width=300,
                value=str(agent.get("azure_endpoint") or ""),
                hint_text="https://{resource}.openai.azure.com",
            )
            azure_api_version_field = ft.TextField(
                label="azure_api_version",
                width=300,
                value=str(agent.get("azure_api_version") or ""),
            )
            deployment_name_field = ft.TextField(
                label="deployment_name (optional)",
                width=300,
                value=str(agent.get("deployment_name") or ""),
                hint_text="If empty, model_id is used",
            )
            ...
            def save_edit(ev) -> None:
                try:
                    ...
                    _update_agent_config_fields(
                        ...
                        provider_id=provider_id_field.value,
                        azure_endpoint=azure_endpoint_field.value,
                        azure_api_version=azure_api_version_field.value,
                        deployment_name=deployment_name_field.value,
                    )
            ...
            dlg = ft.AlertDialog(
                ...
                content=ft.Column(
                    [
                        ...
                        model_id_field,
                        provider_id_field,
                        azure_endpoint_field,
                        azure_api_version_field,
    ...
    def create_agent_action(
        ...
        try:
            ...
            _update_agent_config_fields(
                ...
                azure_api_version=azure_api_version_val,
                deployment_name=deployment_name_val,
            )
    ...
    def show_add_dialog(e):
        ...
        provider_id_field = ft.TextField(label="provider_id (optional)", width=300)
        azure_endpoint_field = ft.TextField(
            label="azure_endpoint",
            width=300,
            hint_text="https://{resource}.openai.azure.com",
            ...
            actions=[
                ...
                ft.ElevatedButton(
                    ...
                    on_click=lambda e: create_agent_action(
                        ...
                        provider_id_field.value,
                        azure_endpoint_field.value,
                        azure_api_version_field.value,
                        deployment_name_field.value,
                        creds_field.value,
                        new_dialog,
                    ),
                    bgcolor="#2196F3",
